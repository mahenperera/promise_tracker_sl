...
export default function RatingWidget({ promiseId }) {
    ...
    if (loading) return <div className="text-sm text-slate-500">Loading rating...</div>;
    ...
    return (
        <div className="rounded-2xl border border-slate-200 bg-white p-6 mb-6 shadow-sm">
            <div className="flex flex-col sm:flex-row items-baseline sm:items-center justify-between gap-4">
                <div>
                    <h3 className="text-lg font-semibold text-slate-900">Community Rating</h3>
                    <p className="text-sm text-slate-500">{avg.toFixed(1)} out of 5 ({count} ratings)</p>
                </div>
                <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        >
                            <span className={star <= (hovered || avg) ? "text-amber-400" : "text-slate-200"}>
                                ★
                            </span>
                        </button>
                    ))}
